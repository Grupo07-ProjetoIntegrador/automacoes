    ...
    for p in participantes:
        ...
        linhas_tabela.append(f"""
            <tr>
                ...
                <td style="padding: 12px 15px; color: #4B5563; font-weight: 500;">{p.get('loja')}</td>
...
<head>
    ...
    <style>
        * {{
        ...
        /* CONTAINER MESTRE DE SEGURANÇA (Preenche 100% sem quebrar o QPainter) */
        .wrapper-page {{
            width: 100%;
            padding: 30px 35px;
        }}
        ...
        /* GRID DE INFORMAÇÕES - FORMATO DE TABELA ULTRA ESTÁVEL */
        .info-table-grid {{
            width: 100%;
        ...
        /* LINHA DE ALINHAMENTO DA RELAÇÃO NOMINAL */
        .tabela-titulo {{
            color: #1F2937;
            font-size: 11.5px;
            text-transform: uppercase;
...
<body>
    ...
    <div class="wrapper-page">
        ...
        <table class="info-table-grid">
            <tr>
                <td class="info-col" style="width: 54%;">
                    <div class="secao-titulo">Detalhes do Treinamento</div>
                    <table style="width: 100%; font-size: 12.5px; border-collapse: collapse;">
                        <tr>
                            <td style="padding: 4px 0; font-weight: 700; color: #1F2937; width: 32%; border: none;">Módulo / Tema:</td>
                ...
                <td style="width: 3%; background-color: transparent; border: none;"></td>
                <td class="info-col" style="width: 43%;">
                    <div class="secao-titulo">Performance da Sessão</div>
                    <div style="display: table; width: 100%; margin-top: 5px;">
                        <div style="display: table-cell; width: 45%; vertical-align: middle;">
                            <div style="font-size: 34px; font-weight: 800; color: #8B1A1A; line-height: 1;">{taxa_presenca}%</div>
                            <div style="font-size: 11px; color: #6B7280; margin-top: 4px; font-style: italic;">Taxa de Presença</div>
                        </div>
                        <div style="display: table-cell; width: 55%; vertical-align: middle; padding-left: 12px; border-left: 1px solid #E5E7EB; font-size: 11.5px; color: #4B5563; line-height: 1.4;">
                            <div style="margin-bottom: 2px;">Convocados: <strong>{total_convocados}</strong></div>
                            <div style="margin-bottom: 2px; color: #10B981;">Presentes: <strong>{total_presentes}</strong></div>
                            <div style="color: #D93030;">Faltas: <strong>{total_ausentes} ({taxa_absenteismo}%)</strong></div>
                        </div>
                    </div>
                    <div style="width: 100%; background-color: #E5E7EB; height: 5px; border-radius: 3px; margin-top: 15px; overflow: hidden;">
...
if __name__ == "__main__":
    ...
    with open(nome_arquivo_html, "w", encoding="utf-8") as f:
        f.write(html_final)
    print(f"✔ Arquivo temporário '{nome_arquivo_html}' atualizado.")
    ...
    if PDFKIT_DISPONIVEL:
        ...
        try:
            ...
            # REMOVEMOS MARGENS MILIMÉTRICAS PARA EVITAR O ERRO QPAINTER:
            # Passamos apenas o page-size estrito e ativamos o quiet mode.
            opcoes = {
                'page-size': 'A4',
                'encoding': "UTF-8",
                'quiet': '',
            ...
            print(f"✔ PDF '{nome_arquivo_pdf}' gerado com sucesso e alinhamento perfeito!")
        except Exception as e:
